...
import { CalendarDays, ChevronLeft, ChevronRight, FileText, Mail, Package, Pill, RefreshCw, ShieldAlert, ShieldCheck } from "lucide-react-native";
...
export const CaregiverPatientDetailScreen = ({ navigation, route }: CaregiverPatientDetailScreenProps) => {
  ...
  const loadPatient = useCallback(async (mode: "initial" | "refresh" = "initial") => {
    try {
      ...
      setErrorMessage("");
      setData(await caregiverPatientsApi.getLinkedPatient(route.params.patientId));
    } catch (error) {
  ...
  const patientName = data?.patient.fullName || route.params.patientName;
  ...
  const openMedicines = () => navigation.navigate("CaregiverMedications", { patientId: route.params.patientId, patientName });
  const openOrders = () => navigation.navigate("CaregiverPharmacyOrders", { patientId: route.params.patientId, patientName });
  const openObservations = () => navigation.navigate("CaregiverObservations", { patientId: route.params.patientId, patientName });
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      <View style={styles.screen}>
        ...
        >
          {isLoading ? <View style={styles.stateContainer}><ActivityIndicator color={PRIMARY} /><Text style={styles.stateTitle}>Loading patient</Text></View> : null}

          {!isLoading && errorMessage ? (
            <View style={styles.stateContainer}>
              <View style={styles.errorIcon}><RefreshCw size={24} color={DANGER} strokeWidth={2.5} /></View>
              <Text style={styles.stateTitle}>Couldn't load patient</Text>
              <Text style={styles.stateText}>{errorMessage}</Text>
              <TouchableOpacity style={styles.retryButton} onPress={() => void loadPatient("initial")}><Text style={styles.retryText}>Try again</Text></TouchableOpacity>
            </View>
          ) : null}
            ...
            <>
              <LinearGradient colors={[PRIMARY, PRIMARY_SECONDARY]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.heroCard}>
                ...
                <View style={styles.heroTop}>
                  <View style={styles.avatar}><Text style={styles.avatarText}>{getInitials(data.patient.fullName)}</Text></View>
                  <View style={styles.linkedBadge}><ShieldCheck size={14} color={SURFACE} strokeWidth={2.5} /><Text style={styles.linkedBadgeText}>Linked</Text></View>
                </View>
                ...
                <View style={styles.heroFooter}><View style={styles.activeDot} /><Text style={styles.heroFooterText}>{formatLinkedDate(data.relationship.linkedAt)}</Text></View>
              </LinearGradient>

              <View style={styles.accessCard}>
                <View style={styles.accessIcon}><ShieldCheck size={23} color={SUCCESS_DARK} strokeWidth={2.6} /></View>
              ...
              <TouchableOpacity style={styles.medicineCard} activeOpacity={0.86} onPress={openMedicines}>
                <View style={styles.largeCardTop}>
                  <View style={styles.medicineIcon}><Pill size={26} color={PRIMARY_DARK} strokeWidth={2.6} /></View>
                  <View style={styles.cardArrow}><ChevronRight size={20} color={PRIMARY_DARK} strokeWidth={2.5} /></View>
                </View>

                <Text style={styles.largeCardTitle}>Medicines</Text>
                <Text style={styles.largeCardSubtitle}>View schedules, dose status and stock levels</Text>
                <View style={styles.largeCardFooter}><Text style={styles.medicineFooterText}>Medication support</Text></View>
              </TouchableOpacity>

              <TouchableOpacity style={styles.ordersCard} activeOpacity={0.86} onPress={openOrders}>
                <View style={styles.largeCardTop}>
                  <View style={styles.ordersIcon}><Package size={26} color={SUCCESS_DARK} strokeWidth={2.6} /></View>
                  <View style={styles.ordersArrow}><ChevronRight size={20} color={SUCCESS_DARK} strokeWidth={2.5} /></View>
                </View>

                <Text style={styles.largeCardTitle}>Pharmacy orders</Text>
                <Text style={styles.ordersSubtitle}>Track medicine fulfilment, payment and delivery progress</Text>
                <View style={styles.ordersFooter}><Text style={styles.ordersFooterText}>Order tracking</Text></View>
              </TouchableOpacity>

              <TouchableOpacity style={styles.observationsCard} activeOpacity={0.86} onPress={openObservations}>
                <View style={styles.observationsIcon}><FileText size={24} color={PRIMARY_DARK} strokeWidth={2.6} /></View>

                <View style={styles.observationsText}>
                  <Text style={styles.observationsTitle}>Care observations</Text>
                  <Text style={styles.observationsSubtitle}>Record everyday non-clinical care notes</Text>
                </View>

                <ChevronRight size={19} color={PRIMARY_DARK} strokeWidth={2.5} />
              </TouchableOpacity>

              <View style={styles.twoColumn}>
                <TouchableOpacity style={styles.safetyCard} activeOpacity={0.86} onPress={() => openCaregiverTab("Safety")}>
                  <View style={styles.safetyIcon}><ShieldAlert size={24} color={DANGER} strokeWidth={2.6} /></View>
                  <Text style={styles.smallCardTitle}>Safety</Text>
                  <Text style={styles.smallCardSubtitle}>Alerts and patient status</Text>
                  <View style={styles.smallCardFooter}><Text style={styles.safetyFooterText}>View alerts</Text><ChevronRight size={16} color={DANGER_DARK} strokeWidth={2.5} /></View>
                </TouchableOpacity>

                <TouchableOpacity style={styles.appointmentCard} activeOpacity={0.86} onPress={() => openCaregiverTab("Appointments")}>
                  <View style={styles.appointmentIcon}><CalendarDays size={24} color={BLUE} strokeWidth={2.6} /></View>
                  <Text style={styles.smallCardTitle}>Appointments</Text>
                  <Text style={styles.smallCardSubtitle}>Consultations and requests</Text>
                  <View style={styles.smallCardFooter}><Text style={styles.appointmentFooterText}>View schedule</Text><ChevronRight size={16} color={BLUE} strokeWidth={2.5} /></View>
                </TouchableOpacity>
              </View>

              <View style={styles.supportPanel}>
                <View style={styles.supportIcon}><ShieldCheck size={20} color={PRIMARY_DARK} strokeWidth={2.5} /></View>
...
const styles = StyleSheet.create({
  ...
  heroCard: { minHeight: 235, borderRadius: 24, padding: 20, overflow: "hidden" },
  ...
  patientName: { color: SURFACE, fontSize: 25, fontWeight: "800", marginTop: 21 },
  ...
  accessTextBlock: { flex: 1 },
  accessTitle: { color: SUCCESS_DARK, fontSize: 13, fontWeight: "800" },
  accessSubtitle: { color: "#4D806D", fontSize: 9, fontWeight: "600", marginTop: 3 },
  ...
  medicineCard: { minHeight: 176, borderRadius: 20, backgroundColor: PRIMARY_LIGHT, padding: 17, borderWidth: 1, borderColor: "#FBE1B9" },
  ordersCard: { minHeight: 164, borderRadius: 20, backgroundColor: SUCCESS_LIGHT, padding: 17, borderWidth: 1, borderColor: "#CEEBDD", marginTop: 11 },
  largeCardTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  medicineIcon: { width: 52, height: 52, borderRadius: 16, backgroundColor: SURFACE, alignItems: "center", justifyContent: "center" },
  ordersIcon: { width: 52, height: 52, borderRadius: 16, backgroundColor: SURFACE, alignItems: "center", justifyContent: "center" },
  ...
  ordersArrow: { width: 38, height: 38, borderRadius: 12, backgroundColor: SURFACE, alignItems: "center", justifyContent: "center" },
  largeCardTitle: { color: TEXT, fontSize: 18, fontWeight: "800", marginTop: 17 },
  largeCardSubtitle: { color: "#7F6B53", fontSize: 10, lineHeight: 16, fontWeight: "600", marginTop: 4 },
  ordersSubtitle: { color: "#4D806D", fontSize: 10, lineHeight: 16, fontWeight: "600", marginTop: 4 },
  largeCardFooter: { alignSelf: "flex-start", backgroundColor: SURFACE, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 5, marginTop: 12 },
  medicineFooterText: { color: PRIMARY_DARK, fontSize: 8, fontWeight: "800" },
  ordersFooter: { alignSelf: "flex-start", backgroundColor: SURFACE, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 5, marginTop: 12 },
  ordersFooterText: { color: SUCCESS_DARK, fontSize: 8, fontWeight: "800" },

  observationsCard: { minHeight: 82, borderRadius: 18, backgroundColor: "#FFF8EE", borderWidth: 1, borderColor: "#FBE5C7", padding: 13, flexDirection: "row", alignItems: "center", marginTop: 11 },
  observationsIcon: { width: 48, height: 48, borderRadius: 14, backgroundColor: SURFACE, alignItems: "center", justifyContent: "center", marginRight: 11 },
  observationsText: { flex: 1, paddingRight: 8 },
  observationsTitle: { color: TEXT, fontSize: 13, fontWeight: "800" },
  observationsSubtitle: { color: MUTED, fontSize: 9, lineHeight: 14, fontWeight: "600", marginTop: 3 },
  ...
  supportSubtitle: { color: MUTED, fontSize: 9, fontWeight: "600", marginTop: 3 },

  stateContainer: { backgroundColor: SURFACE, borderRadius: 18, alignItems: "center", paddingVertical: 70, paddingHorizontal: 24 },
  stateTitle: { color: TEXT, fontSize: 15, fontWeight: "700", marginTop: 11 },
  stateText: { color: MUTED, fontSize: 11, lineHeight: 17, textAlign: "center", marginTop: 5 },
