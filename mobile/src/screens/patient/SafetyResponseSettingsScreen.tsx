...
const SafetyResponseSettingsScreen = ({ navigation }: Props) => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Unable to load Safety Response settings.",
      );
    } finally {
      setIsLoading(false);
  ...
  const toggle = (key: SafetyBooleanKey) => {
    setSettings(current => ({
      ...current,
      [key]: !current[key],
    }));
  ...
  const saveSettings = async () => {
    try {
      ...
      Alert.alert(
        ...
        `New Safety Responses will use a ${result.settings.countdownSeconds}-second countdown.`,
      );
    } catch (error) {
      Alert.alert(
      ...
      <View style={styles.screen}>
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={styles.headerSubtitle}>Emergency escalation preferences</Text>
          </View>
        </View>
        ...
        <ScrollView
          ...
          contentContainerStyle={[
            ...
            { paddingBottom: Math.max(36, insets.bottom + 26) },
          ]}
          showsVerticalScrollIndicator={false}
          refreshControl={
            <RefreshControl
              refreshing={isRefreshing}
              onRefresh={() => void loadSettings("refresh")}
              colors={[DANGER]}
              tintColor={DANGER}
            />
          }
        >
          {isLoading ? (
            <View style={styles.stateCard}>
              ...
              <Text style={styles.stateText}>Loading Safety Response settings...</Text>
            </View>
          ) : null}
            ...
            <View style={styles.errorCard}>
              ...
              <View style={styles.errorContent}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => void loadSettings("initial")}
                >
                  <RefreshCw size={16} color={SURFACE} />
            ...
            <>
              <View style={styles.heroCard}>
                <View style={styles.heroIcon}>
                  <ShieldAlert size={25} color={DANGER} strokeWidth={2.5} />
                </View>

                <View style={styles.heroText}>
                  <Text style={styles.heroTitle}>Emergency countdown</Text>
                  <Text style={styles.heroDescription}>
                    The selected countdown is used by the next newly created Safety Response alert.
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.section}>
                ...
                <Text style={styles.sectionSubtitle}>
                  Time before automatic emergency escalation
                </Text>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        setSettings(current => ({
                          ...current,
                          countdownSeconds:
                            option as SafetySettings["countdownSeconds"],
                        }))
                      }
                    >
                      <Clock3
              ...
              <View style={styles.section}>
                ...
                <Text style={styles.sectionSubtitle}>
                  Control primary-doctor notification and clinical information shared during escalation
                </Text>

                <ToggleRow
                  icon={<Stethoscope size={20} color={DANGER} />}
                  title="Notify primary doctor"
                  description="Send the Safety Response escalation notification only to your active primary doctor."
                  value={settings.notifyAssignedDoctors}
                  onPress={() => toggle("notifyAssignedDoctors")}
                />

                <ToggleRow
                  icon={<HeartPulse size={20} color={DANGER} />}
                  title="Share latest vital reading"
                  description="Include the critical reading with the primary doctor's emergency consultation."
                  value={settings.shareLatestVitalsOnEscalation}
                  onPress={() => toggle("shareLatestVitalsOnEscalation")}
                />

                <ToggleRow
                  icon={<ContactRound size={20} color={DANGER} />}
                  title="Notify emergency contact"
                  description="Save permission for emergency-contact notifications."
                  value={settings.notifyEmergencyContact}
                  onPress={() => toggle("notifyEmergencyContact")}
                  isLast
                />
              </View>
              ...
              <View style={styles.warningCard}>
                ...
                <Text style={styles.warningText}>
                  Primary-doctor notification and latest-vital sharing are applied during Safety Response escalation. Emergency-contact notification is currently stored as a preference but is not delivered automatically.
                </Text>
              </View>
              ...
              >
                {isSaving ? (
                  ...
                  <>
                    <Save size={19} color={SURFACE} />
                    <Text style={styles.saveText}>Save Safety Settings</Text>
                  </>
                )}
              </TouchableOpacity>
            </>
