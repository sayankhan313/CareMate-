...
type AdminPharmacyListScreenProps = CompositeScreenProps<
  BottomTabScreenProps<AdminTabParamList, "Pharmacies">,
...
const getSummaryText = (status: AdminAccountStatus) => {
  if (status === "ACTIVE" || status === "APPROVED") {
    return "These pharmacies are approved and active in CareMate+.";
  }

  if (status === "REJECTED") {
    return "These pharmacy verification requests were rejected.";
  }

  if (status === "DISABLED") {
    return "These pharmacy accounts are currently disabled.";
  }

  return "Review pharmacy licence, address proof and registration details.";
};

export const AdminPharmacyListScreen = ({
  navigation,
  route,
}: AdminPharmacyListScreenProps) => {
  const insets = useSafeAreaInsets();

  const status = route.params?.status || "PENDING_VERIFICATION";

  const [pharmacies, setPharmacies] = useState<AdminPharmacyVerification[]>([]);
  ...
  const loadPharmacies = useCallback(
    async (mode: "initial" | "refresh" = "initial") => {
      try {
        ...
        const result = await adminApi.listPharmacyVerifications(status);
  ...
  useFocusEffect(
    useCallback(() => {
      loadPharmacies("initial");
    }, [loadPharmacies])
  );

  const openPharmacy = (pharmacyId: string) => {
  ...
  const renderPharmacyCard = (pharmacy: AdminPharmacyVerification) => {
    const tone = getStatusTone(pharmacy.accountStatus);
    ...
    return (
      <TouchableOpacity
        ...
        style={styles.card}
        activeOpacity={0.86}
        onPress={() => openPharmacy(pharmacy.id)}
      >
          ...
          <View style={styles.footerRow}>
            <View style={styles.docChip}>
              <BadgeCheck
                ...
                strokeWidth={2.5}
              />
              <Text style={styles.docChipText}>Licence docs</Text>
            </View>
      ...
      <View style={styles.screen}>
        <View style={styles.topBar}>
          <View style={styles.titleIcon}>
            <Building2 size={22} color={ON_ADMIN_CONTAINER} strokeWidth={2.5} />
          </View>
          ...
          <TouchableOpacity
            ...
            onPress={() => loadPharmacies("initial")}
            activeOpacity={0.82}
          >
        ...
        <ScrollView
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={() => loadPharmacies("refresh")}
              tintColor={ADMIN}
              colors={[ADMIN]}
            />
          }
        ...
        >
          {isLoading ? (
            <View style={styles.stateCard}>
              ...
              <Text style={styles.stateText}>Loading pharmacies...</Text>
            </View>
          ) : errorMessage ? (
            <View style={styles.stateCard}>
              ...
              <Text style={styles.stateTitle}>Unable to load pharmacies</Text>
              ...
              <View style={styles.summaryCard}>
                ...
                <View style={styles.summaryTextBlock}>
                  <Text style={styles.summaryTitle}>
                    {pharmacies.length} pharmacy account
            ...
            <View style={styles.stateCard}>
              ...
              <Text style={styles.stateTitle}>No pharmacies found</Text>
              <Text style={styles.stateText}>
                There are no pharmacy accounts in this filter.
              </Text>
            </View>
          )}
        </ScrollView>
