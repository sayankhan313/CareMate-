...
const resolveReportId = (report: RuntimeReport) => {
  return getSafeString(report.id) || getSafeString(report.reportId) || getSafeString(report.report?.id);
};
...
export const DoctorPatientReportsScreen = ({ navigation, route }: Props) => {
  ...
  const visibleReports = useMemo(() => {
    ...
    return reports.filter((report) => {
      const status = report.review?.status || "PENDING";
  ...
  const loadReports = useCallback(
    async (mode: "initial" | "refresh" = "initial") => {
      ...
      try {
        ...
        console.log("DOCTOR REPORT LIST:", JSON.stringify(receivedReports, null, 2));

        const correctedReports = receivedReports.map((report) => {
          const runtimeReport = report as RuntimeReport;
          const resolvedId = resolveReportId(runtimeReport);
        ...
        setErrorMessage(
          error instanceof Error ? error.message : "Unable to load patient reports."
        );
      } finally {
        setIsLoading(false);
        setIsRefreshing(false);
  ...
  const openReport = (report: DoctorPatientReport) => {
    ...
    const resolvedPatientId =
      getSafeString(report.patientId) ||
      getSafeString(report.patient?.id) ||
      patientId;

    const resolvedPatientName =
      getSafeString(report.patient?.fullName) ||
      patientName;

    console.log("OPEN REPORT PRESSED:", {
    ...
    if (!resolvedPatientId) {
      Alert.alert(
        ...
        "Patient ID is missing. Please reopen this patient from Assigned Patients."
      );
      return;
    }
    ...
    if (!resolvedReportId) {
      Alert.alert(
        "Unable to open report",
        "The backend did not return the medical report ID."
      );
      return;
    }

    navigation.dispatch(
      StackActions.push("DoctorReportReview", {
        patientId: resolvedPatientId,
        patientName: resolvedPatientName,
        reportId: resolvedReportId,
      })
            ...
            <View style={styles.summaryHeader}>
              ...
              <View style={styles.summaryTextBlock}>
                ...
                <Text style={styles.summaryText}>
                  Review protected reports uploaded by this patient.
                </Text>
              </View>
            </View>
          ...
          {isLoading ? (
            <View style={styles.stateCard}>
              ...
              <Text style={styles.stateText}>
                Fetching protected patient report records.
              </Text>
            </View>
            ...
            <View style={styles.stateCard}>
              ...
              <Text style={styles.stateText}>
                Patient reports matching this filter will appear here.
              </Text>
            </View>
          ) : (
            visibleReports.map((report, index) => (
              <ReportCard
                key={getSafeString(report.id) || `report-${index}`}
                report={report}
    ...
    <View style={styles.reportCard}>
      ...
      <View style={styles.reportHeader}>
        ...
        >
          {reviewStatus === "REVIEWED" ? (
            <CheckCircle2 size={14} color={reviewTone.text} strokeWidth={2.7} />
          ) : (
            <Clock3 size={14} color={reviewTone.text} strokeWidth={2.7} />
          )}

          <Text style={[styles.reviewBadgeText, { color: reviewTone.text }]}>
            {reviewTone.label}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.reportFooter}>
        ...
        >
          ...
          <Text style={[styles.safetyText, { color: safetyTone.text }]}>
            {safetyTone.label}
          </Text>
        </View>
      </View>
      ...
      >
        <Text style={styles.openReportButtonText}>
          {reviewStatus === "REVIEWED" ? "View Review" : "Open Report"}
        </Text>
...
const styles = StyleSheet.create({
  ...
  filterChipSelected: {
    backgroundColor: DOCTOR_PRIMARY_LIGHT,
  },
