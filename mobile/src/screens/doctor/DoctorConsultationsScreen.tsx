...
import {
  ...
  type DoctorConsultationFilterStatus,
  type DoctorConsultationFilterType,
} from "../../services/doctor/doctorConsultationsApi";
...
const STATUS_FILTERS: {
  label: string;
  value: DoctorConsultationFilterStatus;
}[] = [
  { label: "All", value: "ALL" },
...
const TYPE_FILTERS: {
  label: string;
  value: DoctorConsultationFilterType;
}[] = [
  { label: "All", value: "ALL" },
...
const elevate = (level: 1 | 2 | 3 = 1) => {
  ...
  return {
    elevation,
    shadowColor: "#172033",
    shadowOpacity: Platform.OS === "android" ? 0 : 0.09,
...
const getPatientName = (consultation: DoctorConsultation) => {
  return consultation.patient?.fullName || "Assigned patient";
};

const canAccept = (consultation: DoctorConsultation) => {
  return consultation.status === "PENDING";
};

const canReject = (consultation: DoctorConsultation) => {
  return consultation.status === "PENDING" || consultation.status === "ACCEPTED";
};

const canJoin = (consultation: DoctorConsultation) => {
  return (
    consultation.status === "ACCEPTED" || consultation.status === "IN_PROGRESS"
  );
};

const canComplete = (consultation: DoctorConsultation) => {
  return (
    consultation.status === "ACCEPTED" || consultation.status === "IN_PROGRESS"
  );
};
...
export const DoctorConsultationsScreen = ({ navigation }: Props) => {
  ...
  const [selectedStatus, setSelectedStatus] =
    useState<DoctorConsultationFilterStatus>("ALL");
  const [selectedType, setSelectedType] =
    useState<DoctorConsultationFilterType>("ALL");
  ...
  const loadConsultations = useCallback(
    async (mode: "initial" | "refresh" = "initial") => {
      try {
        ...
        const result = await doctorConsultationsApi.listConsultations(
          selectedStatus,
          selectedType
        );
  ...
  const updateConsultationInList = (updatedConsultation: DoctorConsultation) => {
    setConsultations((currentConsultations) => {
      return currentConsultations.map((consultation) => {
  ...
  const joinConsultationCall = async (consultation: DoctorConsultation) => {
    ...
    try {
      ...
      const result = await doctorConsultationsApi.getDoctorJoinConfig(
        consultation.id
      );
  ...
  const submitAction = async () => {
    if (!selectedConsultation || !selectedAction || actionLoadingId) {
      return;
    }
    ...
    try {
      ...
      if (selectedAction === "ACCEPT") {
        result = await doctorConsultationsApi.acceptConsultation(
          selectedConsultation.id,
          actionNotes
        );
      } else if (selectedAction === "REJECT") {
        result = await doctorConsultationsApi.rejectConsultation(
          selectedConsultation.id,
          actionNotes
        );
      } else {
        result = await doctorConsultationsApi.completeConsultation(
          selectedConsultation.id,
          actionNotes
        );
      }
      ...
      Alert.alert("Consultation update failed", message);
    } finally {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      <View style={styles.screen}>
        <View style={styles.appBar}>
          <View>
            ...
            <Text style={styles.appBarSubtitle}>
              Review patient requests and join calls
            </Text>
          </View>
        ...
        >
          ...
          <View style={styles.summaryCard}>
            ...
            <View style={styles.summaryTopRow}>
              <View style={styles.summaryIconRingOuter}>
                <View style={styles.summaryIconRingInner}>
                  <Video size={22} color={DOCTOR_PRIMARY} strokeWidth={2.7} />
                </View>
              </View>
              ...
              <View style={styles.summaryTextBlock}>
                ...
                <Text style={styles.summaryText}>
                  Accept patient requests, join active video calls, and
              ...
              {STATUS_FILTERS.map((filter) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.materialChip,
                      isSelected ? styles.materialChipSelected : undefined,
                    ]}
                    activeOpacity={0.7}
                  ...
                  >
                    ...
                    <Text
                      style={[
                        ...
                        isSelected
                          ? styles.materialChipTextSelected
                          : undefined,
                      ]}
                    >
                      {filter.label}
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      isSelected ? styles.segmentedItemSelected : undefined,
                      !isLast ? styles.segmentedItemDivider : undefined,
                    ]}
                    activeOpacity={0.7}
                  ...
                  >
                    <Text
                      style={[
                        ...
                        isSelected
                          ? styles.segmentedItemTextSelected
                          : undefined,
                      ]}
                    >
                      {filter.label}
          ...
          {screenError ? (
            <View style={styles.errorCard}>
              <AlertTriangle size={22} color={DANGER} strokeWidth={2.7} />
            ...
            <View style={styles.listStack}>
              {consultations.map((consultation) => (
                <ConsultationCard
                  ...
                  onJoin={() => joinConsultationCall(consultation)}
                />
              ))}
            </View>
          )}
        ...
        <Modal
          visible={Boolean(selectedConsultation && selectedAction)}
          transparent
          animationType="fade"
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              <View style={styles.modalHeader}>
                ...
                >
                  {selectedAction === "REJECT" ? (
                    <XCircle size={24} color={DANGER} strokeWidth={2.7} />
                  ) : selectedAction === "COMPLETE" ? (
                    <CheckCircle2 size={24} color={SUCCESS} strokeWidth={2.7} />
                  ) : (
                    <Stethoscope
                      size={24}
                      color={DOCTOR_PRIMARY}
              ...
              <TextInput
                ...
                placeholder="Add a short message or clinical note..."
              ...
              <View style={styles.modalButtonRow}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    actionLoadingId ? styles.disabledAction : undefined,
                  ]}
                  activeOpacity={0.86}
...
const EmptyCard = () => {
  return (
    <View style={styles.stateCard}>
      <View style={styles.emptyIconRingOuter}>
      ...
      <Text style={styles.stateTitle}>No consultation requests</Text>
      <Text style={styles.stateText}>
        Patient manual and emergency requests will appear here.
      </Text>
    </View>
  );
};
...
const ConsultationCard = ({
  ...
  return (
    <View style={styles.consultationCard}>
      <View
        style={[
          styles.consultationAccentBar,
          { backgroundColor: isEmergency ? DANGER : tone.solid },
        ]}
      />
      ...
      <View style={styles.consultationTopRow}>
        <View
          style={[
            ...
            isEmergency ? styles.emergencyIconBox : undefined,
          ]}
        >
          {isEmergency ? (
            <AlertTriangle size={22} color={DANGER} strokeWidth={2.7} />
          ) : (
            <UserRound size={22} color={DOCTOR_PRIMARY} strokeWidth={2.7} />
          )}
        </View>
        ...
        <View style={styles.consultationTitleBlock}>
          ...
          <Text style={styles.consultationMeta} numberOfLines={1}>
      ...
      <View style={styles.infoPanel}>
        <InfoLine
          ...
          label="Preferred"
          value={formatDateTime(consultation.preferredAt)}
        />

        <InfoLine
          icon={<Clock3 size={16} color={DOCTOR_PRIMARY} strokeWidth={2.5} />}
          label="Requested"
      ...
      <View style={styles.reasonPanel}>
        <View style={styles.reasonHeader}>
          <FileText size={16} color={MUTED} strokeWidth={2.5} />
      ...
      <View style={styles.actionRow}>
        {canAccept(consultation) ? (
          <TouchableOpacity
            style={[
              styles.acceptButton,
              isLoading ? styles.disabledAction : undefined,
            ]}
            activeOpacity={0.86}
            onPress={onAccept}
            disabled={isLoading}
        ...
        {canReject(consultation) ? (
          <TouchableOpacity
            ...
            disabled={isLoading}
          >
            <XCircle size={16} color={DANGER_DARK} strokeWidth={2.6} />
        ...
        {canJoin(consultation) ? (
          <TouchableOpacity
            style={[
              styles.joinButton,
              isJoining ? styles.disabledAction : undefined,
            ]}
            activeOpacity={0.86}
            onPress={onJoin}
            disabled={isJoining}
          >
            {isJoining ? (
              ...
              <>
                <Video size={16} color="#FFFFFF" strokeWidth={2.6} />
        ...
        !canComplete(consultation) ? (
          <View style={styles.noActionBox}>
            <History size={16} color={MUTED} strokeWidth={2.5} />
...
const styles = StyleSheet.create({
  ...
  // Material 3 "filter chip": outlined + transparent when unselected,
  // tonal container fill with a leading checkmark when selected — a quiet,
  // native Android control rather than a bold solid-fill chip.
  materialChip: {
    flexDirection: "row",
    alignItems: "center",
  ...
  // Material 3 "segmented button" group: one continuous outlined pill split
  // into equal segments by hairline dividers — the Android pattern for
  // exclusive choice between a small, fixed set of options.
  segmentedGroup: {
    flexDirection: "row",
    borderWidth: 1,
    borderColor: "#C7CCDA",
  ...
  segmentedItemDivider: {
    borderRightWidth: 1,
    borderRightColor: "#C7CCDA",
  },
