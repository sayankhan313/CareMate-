...
import { doctorRefillVerificationsApi } from "../../services/doctor/doctorRefillVerificationsApi";
...
export const DoctorDashboardScreen = ({
  ...
  const [pendingReportReviews, setPendingReportReviews] =
    useState(0);

  const [pendingPoolReviews, setPendingPoolReviews] =
    useState(0);

  const [
    pendingRefillVerifications,
    setPendingRefillVerifications,
  ] = useState(0);
  ...
  const [joiningAlertId, setJoiningAlertId] =
    useState<string | null>(null);

  const [acceptingAlertId, setAcceptingAlertId] =
    useState<string | null>(null);

  const routeUser = route.params?.user;

  const doctorName =
    dashboard?.doctor.fullName ||
    routeUser?.fullName ||
    "";
    ...
    async (
      ...
      try {
        ...
        const [
          ...
          reportQueue,
          poolReviewQueue,
          refillVerificationQueue,
          notificationData,
        ] = await Promise.all([
          doctorDashboardApi.getDashboard(),

          doctorReportsApi
            .listReportQueue("PENDING")
            .catch(() => null),
        ...
        setPendingReportReviews(
          reportQueue?.summary?.pending || 0,
        );

        setPendingPoolReviews(
          poolReviewQueue?.summary
            ?.awaitingReview || 0,
        );

        setPendingRefillVerifications(
          refillVerificationQueue?.count || 0,
        );
        ...
        setPendingRefillVerifications(0);
  ...
  const openNotifications = () =>
    openRootScreen("Notifications");

  const openProfile = () =>
    openRootScreen("DoctorProfile");

  const openAvailability = () =>
    openRootScreen("DoctorAvailability");
  ...
  const openNotePatients = () =>
    openRootScreen("DoctorSelectNotePatient");

  const openReportReviews = () =>
    openRootScreen("DoctorReportReviews");

  const openPoolReviews = () =>
    openRootScreen(
      "DoctorMedicineReviewPool",
    );

  const openRefillVerifications = () =>
    openRootScreen(
    ...
    async (
      ...
      if (!alert.consultation) {
        Alert.alert(
          ...
          "This Safety Response does not have a linked emergency consultation.",
        );
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Unable to accept the emergency consultation.";
  ...
  const confirmAcceptAlertConsultation = (
    alert: DoctorUrgentAlert,
  ) => {
    Alert.alert(
  ...
  const joinAlertCall = async (
    ...
    if (!alert.canJoinCall) {
      Alert.alert(
        "Call unavailable",
        "This emergency consultation is not currently available for you to join.",
      );
    ...
    try {
      ...
      const doctorMeetingUrl =
        result.doctorMeeting?.webUrl;

      if (!doctorMeetingUrl) {
        throw new Error(
          "Doctor meeting link was not returned.",
        );
      }
      ...
      if (!rootNavigation) {
        throw new Error(
          "Video consultation screen is not available.",
        );
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to join the emergency consultation.";
              ...
              {dashboard.urgentAlerts
                  ...
                  {dashboard.urgentAlerts.map(
                    alert => (
                      <UrgentAlertPanel
                        ...
                        onAccept={() =>
                          confirmAcceptAlertConsultation(
                            alert,
                          )
                        }
                        onJoin={() =>
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      style={
                        styles.safePanelText
                      }
                    >
                      Critical Safety
                      Response alerts will
                      appear here when a
                      patient escalates.
                    </Text>
                  </View>
  ...
  return (
    ...
    >
      <View style={styles.urgentHeader}>
        ...
        <View
          style={
            styles.urgentTitleBlock
          }
        >
          <Text
            style={styles.urgentTitle}
          >
        ...
        >
          ...
          {alert.canAcceptConsultation ? (
            ...
            <View
              style={[
                styles.alertPrimaryButton,
                styles.alertAwaitingButton,
              ]}
            >
              <Text
                style={
                  styles.alertAwaitingText
                }
              >
                {hasAction
                  ? "Action"
