...
const getImageFileName = (imageUri: string) => {
  ...
  return fileName || `prescription-${Date.now()}.jpg`;
};

const getImageMimeType = (imageUri: string) => {
  const value = imageUri.toLowerCase();
...
export const doctorPrescriptionsApi = {
  async parsePrescriptionScan({
    ...
    return readResponse<ParsedPrescriptionScan>(response);
  },

  async createPrescription(input: CreateDoctorPrescriptionInput) {
    const token = await getToken();
    ...
    if (input.imageUri) {
      ...
      if (input.rawDetectedText?.trim()) {
        formData.append("rawDetectedText", input.rawDetectedText.trim());
      }

      if (input.ocrConfidence !== undefined) {
        formData.append("ocrConfidence", String(input.ocrConfidence));
      }
      ...
      formData.append(
        ...
        {
          uri: input.imageUri,
          name: getImageFileName(input.imageUri),
          type: getImageMimeType(input.imageUri),
        } as any,
      );
    ...
    const response = await fetch(
      ...
      )}/prescriptions`,
      {
        ...
        body: JSON.stringify({
          ...
          rawDetectedText: input.rawDetectedText?.trim() || undefined,
