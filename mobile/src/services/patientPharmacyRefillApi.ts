...
const getErrorMessage = (
  result: ApiEnvelope<unknown> | any,
  fallback: string,
) => {
  ...
  if (Array.isArray(result?.message)) {
    return result.message[0]?.message || fallback;
  }
...
export const patientPharmacyRefillApi = {
  async createRefill(
    ...
    const response = await fetch(
      `${API_BASE_URL}/patient/pharmacy-refills`,
      {
        method: "POST",
        headers: {
    ...
    return readResponse<PharmacyRefillResponse>(
      response,
      "Unable to send medicine request.",
    );
  },
  ...
  async listActiveRefills() {
    ...
    const response = await fetch(
      `${API_BASE_URL}/patient/pharmacy-refills/active`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    return readResponse<ActivePharmacyRefillsResponse>(
      response,
      "Unable to load active pharmacy requests.",
    );
  },
};