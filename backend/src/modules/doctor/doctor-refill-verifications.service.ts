...
const getVerificationRequest = async (
  ...
  const request =
    await prisma.patientPrescriptionSubmission.findFirst({
      where: {
        id: submissionId,
        requestType: "REFILL_REQUEST",
        verificationPath: "ASSIGNED_DOCTOR",
        verificationDoctorId: doctorId,
      },
      select: {
        id: true,
        patientId: true,
  ...
  if (!request) {
    throw new AppError(
      "Medicine verification request not found or it is not assigned to you.",
      404,
    );
  }
...
const notifyPharmacyAfterConfirmation = async ({
  ...
  try {
    ...
    console.warn(
      `Unable to notify pharmacy about doctor-confirmed refill order ${orderId}:`,
...
const notifyPatient = async ({
  ...
  try {
    await notificationService.createAndSend({
      userId: patientId,
      type: confirmed
        ? "REFILL_DOCTOR_VERIFICATION_CONFIRMED"
        : "REFILL_DOCTOR_VERIFICATION_REJECTED",
      title: confirmed
        ? "Medicine request confirmed"
        : "Medicine request not confirmed",
      body: confirmed
    ...
    console.warn(
      `Unable to notify patient about refill verification ${submissionId}:`,
      error instanceof Error ? error.message : error,
    );
  }
};
...
export const doctorRefillVerificationsService = {
  async listPending(doctorId: string) {
    const requests =
      await prisma.patientPrescriptionSubmission.findMany({
        where: {
          requestType: "REFILL_REQUEST",
          verificationPath: "ASSIGNED_DOCTOR",
          verificationDoctorId: doctorId,
        ...
        orderBy: {
          doctorVerificationRequestedAt: "asc",
        },
      });
  ...
  async confirm(
    ...
    if (
      ...
      throw new AppError(
        `This medicine verification request has already been ${request.doctorVerificationStatus.toLowerCase()}.`,
        409,
      );
    }
    ...
    if (!request.medicineOrder) {
      throw new AppError(
        "The pharmacy order linked to this verification request could not be found.",
        404,
      );
    }
    ...
    if (
      TERMINAL_ORDER_STATUSES.includes(
        request.medicineOrder.status as
          (typeof TERMINAL_ORDER_STATUSES)[number],
      )
    ) {
      throw new AppError(
        "This pharmacy order is already closed and can no longer be confirmed.",
        409,
      );
    }
      ...
      async tx => {
        ...
        const order =
          await tx.medicineOrder.update({
            where: {
              id: request.medicineOrder!.id,
            },
            data: {
              doctorId,
              prescriptionConfirmed: true,
              prescriptionConfirmedAt: now,
              fulfilmentAllowed: true,
            },
            select: {
              id: true,
    ...
    return {
      message:
        "Medicine request confirmed successfully.",
      submission:
        updated.submission,
      order: updated.order,
    };
  ...
  async reject(
    ...
    if (
      ...
      throw new AppError(
        "This pharmacy order is already closed.",
        409,
      );
    }
    ...
    const verificationNote =
      note?.trim() ||
      "Selected doctor did not confirm that they prescribed this medicine.";
