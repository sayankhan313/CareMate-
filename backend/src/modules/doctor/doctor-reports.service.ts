...
type ReportQueueInput = {
  status: DoctorReportQueueStatus;
};
...
export const doctorReportsService = {
  ...
  async reviewReport(
    ...
    if (report.contentSafetyStatus === "BLOCKED") {
      throw new AppError("A blocked report cannot be clinically reviewed", 403);
    }
    ...
    await prisma.$transaction(async (transaction) => {
      ...
      let reportStatus: "PENDING_REVIEW" | "PARTIALLY_REVIEWED" | "REVIEWED" =
        "PENDING_REVIEW";

      if (activeDoctorIds.length > 0 && reviewedCount >= activeDoctorIds.length) {
  ...
  async getReportFile(doctorId: string, patientId: string, reportId: string) {
    ...
    if (report.contentSafetyStatus === "BLOCKED") {
      throw new AppError("This medical report file has been blocked", 403);
    }
