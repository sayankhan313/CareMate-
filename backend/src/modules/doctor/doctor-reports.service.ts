...
const ensureReviewRows = async (doctorId: string, reportIds: string[]) => {
  ...
  await prisma.patientReportReview.createMany({
    data: reportIds.map(reportId => ({ reportId, doctorId, status: "PENDING" as const })),
    skipDuplicates: true,
  });
};
...
const formatReport = (report: any) => {
  ...
  return {
    ...
    patient: report.patient ? { id: report.patient.id, fullName: report.patient.fullName, email: report.patient.email } : null,
...
const getReportForDoctor = async (doctorId: string, patientId: string, reportId: string) => {
  await ensureActiveAssignment(doctorId, patientId);

  let report = await prisma.patientReport.findFirst({ where: { id: reportId, patientId }, include: reportInclude(doctorId) });
  ...
  if (report.reviews.length === 0) {
    await prisma.patientReportReview.upsert({
      where: { reportId_doctorId: { reportId, doctorId } },
      update: {},
      create: { reportId, doctorId, status: "PENDING" },
    });

    report = await prisma.patientReport.findFirst({ where: { id: reportId, patientId }, include: reportInclude(doctorId) });
  }
...
const resolveReportFilePath = (storedFileName: string) => {
  ...
  if (!safeFileName || safeFileName !== storedFileName) throw new AppError("Invalid stored report filename", 500);
  ...
  if (!matchedPath) throw new AppError("Stored medical report file was not found", 404);
  return matchedPath;
};
...
const notifyPatientAboutReportReview = async (
  ...
  try {
    ...
    const existingNotification = await prisma.userNotification.findFirst({
      where: { userId: patientId, type: "REPORT_REVIEWED", entityType: "PATIENT_REPORT_REVIEW", entityId: review.id },
      select: { id: true },
    });
    ...
    console.warn(`Unable to notify patient about report review ${report.id}:`, error instanceof Error ? error.message : error);
  }
};
...
export const doctorReportsService = {
  async listReportQueue(doctorId: string, input: ReportQueueInput) {
    ...
    if (assignedPatientIds.length === 0) {
      return { reports: [], summary: { total: 0, pending: 0, reviewed: 0, blocked: 0 } };
    }

    const assignedReports = await prisma.patientReport.findMany({
      where: { patientId: { in: assignedPatientIds } },
      select: { id: true },
    });

    await ensureReviewRows(doctorId, assignedReports.map(report => report.id));

    const reviewFilter = input.status === "ALL" ? { doctorId } : { doctorId, status: input.status };

    const [reports, allDoctorReports] = await Promise.all([
      prisma.patientReport.findMany({
        where: { patientId: { in: assignedPatientIds }, reviews: { some: reviewFilter } },
        include: reportInclude(doctorId),
        orderBy: { createdAt: "desc" },
      }),
      prisma.patientReport.findMany({
        where: { patientId: { in: assignedPatientIds }, reviews: { some: { doctorId } } },
        include: reportInclude(doctorId),
        orderBy: { createdAt: "desc" },
      }),
    ]);
    ...
    return { reports: formattedReports, summary: calculateSummary(formattedAllReports) };
  },

  async listPatientReports(doctorId: string, patientId: string) {
    await ensureActiveAssignment(doctorId, patientId);

    const patientReports = await prisma.patientReport.findMany({ where: { patientId }, select: { id: true } });
    await ensureReviewRows(doctorId, patientReports.map(report => report.id));

    const reports = await prisma.patientReport.findMany({ where: { patientId }, include: reportInclude(doctorId), orderBy: { createdAt: "desc" } });
    const formattedReports = reports.map(formatReport);

    return { reports: formattedReports, summary: calculateSummary(formattedReports) };
  },

  async getReportDetail(doctorId: string, patientId: string, reportId: string) {
    const report = await getReportForDoctor(doctorId, patientId, reportId);
    return formatReport(report);
  },

  async reviewReport(doctorId: string, patientId: string, reportId: string, reviewNote: string) {
    const doctor = await ensureApprovedDoctor(doctorId);
    const report = await getReportForDoctor(doctorId, patientId, reportId);

    if (report.contentSafetyStatus === "BLOCKED") throw new AppError("A blocked report cannot be clinically reviewed", 403);
    ...
    const activeDoctorIds = activeAssignments.map(assignment => assignment.doctorId);

    await prisma.$transaction(async transaction => {
      await transaction.patientReportReview.upsert({
        where: { reportId_doctorId: { reportId, doctorId } },
        update: { status: "REVIEWED", reviewNote, reviewedAt: new Date(), patientSeenAt: null },
        create: { reportId, doctorId, status: "REVIEWED", reviewNote, reviewedAt: new Date() },
      });

      const reviewedCount = await transaction.patientReportReview.count({
        where: { reportId, doctorId: { in: activeDoctorIds }, status: "REVIEWED" },
      });

      let reportStatus: "PENDING_REVIEW" | "PARTIALLY_REVIEWED" | "REVIEWED" = "PENDING_REVIEW";
      if (activeDoctorIds.length > 0 && reviewedCount >= activeDoctorIds.length) reportStatus = "REVIEWED";
      else if (reviewedCount > 0) reportStatus = "PARTIALLY_REVIEWED";

      await transaction.patientReport.update({ where: { id: reportId }, data: { status: reportStatus } });
    });

    const updatedReport = await getReportForDoctor(doctorId, patientId, reportId);

    await notifyPatientAboutReportReview(patientId, updatedReport.patient?.fullName || "Patient", doctor, updatedReport);
    return formatReport(updatedReport);
  },

  async getReportFile(doctorId: string, patientId: string, reportId: string) {
    const report = await getReportForDoctor(doctorId, patientId, reportId);
