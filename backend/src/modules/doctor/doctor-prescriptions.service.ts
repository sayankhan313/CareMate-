...
const ensureApprovedDoctor = async (doctorId: string) => {
  ...
  if (doctor.role !== "DOCTOR") throw new AppError("Only doctors can create prescriptions", 403);
...
const ensureAssignedPatient = async (doctorId: string, patientId: string) => {
  ...
  const assignment = await prisma.patientDoctorAssignment.findFirst({
    where: { doctorId, patientId, status: "ACTIVE", patient: { is: { role: "PATIENT" } } },
    select: {
      id: true,
      assignmentType: true,
      patient: { select: { id: true, fullName: true, email: true } },
  ...
  if (!assignment) throw new AppError("You are not actively assigned to this patient", 403);
  return assignment;
};
...
const parseDateInput = (value: string) => {
  ...
  if (/^\d{2}\/\d{2}\/\d{4}$/.test(value)) {
    ...
    throw new AppError("Date must be in DD/MM/YYYY or YYYY-MM-DD format", 400);
  }

  const parsedDate = new Date(Date.UTC(year, month - 1, day));
  const isInvalid = parsedDate.getUTCFullYear() !== year || parsedDate.getUTCMonth() !== month - 1 || parsedDate.getUTCDate() !== day;

  if (isInvalid) throw new AppError("Please enter a valid prescription date", 400);
  return parsedDate;
};

const getPrescriptionImageUrl = (file: PrescriptionImageFile) => file ? `/uploads/prescriptions/${file.filename}` : null;

const formatPrescription = (prescription: any): DoctorPrescriptionResponse => ({
  id: prescription.id,
  patientId: prescription.patientId,
  prescribedByDoctorId: prescription.prescribedByDoctorId,
  source: prescription.source,
  imageUrl: prescription.imageUrl,
  rawDetectedText: prescription.rawDetectedText,
  ...
  items: prescription.items.map((item: any) => ({
    ...
    prescriptionPattern: item.prescriptionPattern,
...
const getPrescriptionById = async (prescriptionId: string) => {
  return prisma.prescription.findUnique({ where: { id: prescriptionId }, include: prescriptionInclude });
};
...
const notifyPatientAboutPrescription = async (prescription: any) => {
  try {
    const existingNotification = await prisma.userNotification.findFirst({
      where: { userId: prescription.patientId, type: "NEW_PRESCRIPTION", entityType: "PRESCRIPTION", entityId: prescription.id },
      select: { id: true },
    });
    ...
    await notificationService.createAndSend({
      ...
      data: {
        ...
        medicineCount,
        medicineNames,
        prescribedAt: prescription.prescribedAt?.toISOString() || prescription.createdAt.toISOString(),
        source: "DOCTOR_PRESCRIPTION",
      },
    });
  } catch (error) {
    console.warn(`Unable to notify patient about prescription ${prescription.id}:`, error instanceof Error ? error.message : error);
  }
};

export const doctorPrescriptionsService = {
  async parsePrescriptionScan({ doctorId, detectedText, ocrConfidence }: { doctorId: string; detectedText: string; ocrConfidence: number }) {
    await ensureApprovedDoctor(doctorId);
    return medicineReferenceService.parsePrescriptionScan({ detectedText, ocrConfidence });
  },
  ...
  async createPrescription({
    ...
    const parsedItems = input.items.map(item => {
      ...
      if (endDate && endDate < startDate) throw new AppError(`End date cannot be before start date for ${item.name}`, 400);

      return { ...item, startDate, endDate, selectedTimes: Array.from(new Set(item.selectedTimes)) };
    });

    const imageUrl = getPrescriptionImageUrl(imageFile);
    ...
    const prescriptionId = await prisma.$transaction(async tx => {
      const prescription = await tx.prescription.create({
        data: {
          ...
          source: input.source,
          imageUrl,
          rawDetectedText: input.rawDetectedText?.trim() || null,
      ...
      for (const item of parsedItems) {
        ...
        await tx.prescriptionItem.create({
          data: {
            ...
            instructions: item.instructions?.trim() || null,
            frequency: item.frequency,
            customFrequency: item.customFrequency?.trim() || null,
            selectedTimes: item.selectedTimes,
            startDate: item.startDate,
            endDate: item.endDate,
            prescriptionPattern: item.prescriptionPattern?.trim() || null,
          },
        });

        await tx.medicineReminder.createMany({
          data: item.selectedTimes.map(timeOfDay => ({
            medicineId: medicine.id,
            frequency: item.frequency,
            customFrequency: item.customFrequency?.trim() || null,
            timeOfDay,
    ...
    const createdPrescription = await getPrescriptionById(prescriptionId);
    if (!createdPrescription) throw new AppError("Prescription was created but could not be loaded", 500);

    await notifyPatientAboutPrescription(createdPrescription);

    return { prescription: formatPrescription(createdPrescription) };
  },

  async listPatientPrescriptions(doctorId: string, patientId: string): Promise<DoctorPrescriptionsListResponse> {
    await ensureAssignedPatient(doctorId, patientId);
    ...
    return { prescriptions: prescriptions.map(formatPrescription) };
  },

  async getPrescriptionDetail(doctorId: string, prescriptionId: string): Promise<DoctorPrescriptionDetailResponse> {
    await ensureApprovedDoctor(doctorId);

    const prescription = await getPrescriptionById(prescriptionId);
    if (!prescription) throw new AppError("Prescription not found", 404);

    await ensureAssignedPatient(doctorId, prescription.patientId);
