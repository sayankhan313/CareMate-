...
const ensureAssignedPatient = async (doctorId: string, patientId: string) => {
  ...
  if (!assignment) {
    throw new AppError("You are not actively assigned to this patient", 403);
  }
...
const parseDateInput = (value: string) => {
  ...
  if (isInvalid) {
    throw new AppError("Please enter a valid prescription date", 400);
  }

  return parsedDate;
};

const getPrescriptionImageUrl = (file: PrescriptionImageFile) => {
  return file ? `/uploads/prescriptions/${file.filename}` : null;
};
  ...
  async createPrescription({
    ...
    const parsedItems = input.items.map(item => {
      ...
      return {
        ...
        startDate,
        endDate,
        selectedTimes: Array.from(new Set(item.selectedTimes)),
      };
    });
    ...
    const transactionResult = await prisma.$transaction(async tx => {
      ...
      const primaryPharmacyLink = await tx.patientPharmacyLink.findFirst({
        where: {
          patientId,
          isPrimary: true,
        },
      ...
      let pharmacyOrder: RoutedPharmacyOrder = null;

      if (primaryPharmacyLink) {
        const pharmacy = await tx.user.findUnique({
          where: {
            id: primaryPharmacyLink.pharmacyId,
          },
          select: {
            id: true,
            role: true,
        ...
        const pharmacyAvailable =
          ...
          (pharmacy.accountStatus === "ACTIVE" ||
            pharmacy.accountStatus === "APPROVED") &&
          Boolean(pharmacy.pharmacyProfile);

        if (pharmacyAvailable) {
          const existingOrder = await tx.medicineOrder.findFirst({
            where: {
              prescriptionId: prescription.id,
              orderSource: "DOCTOR_PRESCRIPTION",
            },
            ...
            const createdOrder = await tx.medicineOrder.create({
              data: {
                ...
                quantity:
                  prescriptionItems.length === 1
                    ? `${firstItem.quantity} ${
                        firstItem.quantity === "1" ? "pack" : "packs"
                      }`
                ...
                requestedByName: assignment.doctor.fullName,
                requestNote: input.notes?.trim() || null,
    ...
    const createdPrescription = await getPrescriptionById(
    ...
    await notifyPharmacyAboutPrescriptionOrder({
      ...
      order: transactionResult.pharmacyOrder,
    });

    return {
      prescription: formatPrescription(createdPrescription),
    };
  },
