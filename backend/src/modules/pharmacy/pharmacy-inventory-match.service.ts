...
const extractStrength = (name: string, dose?: string | null) => {
  return normalizeStrength(dose) || normalizeStrength(name);
};
...
const getSafeFuzzySimilarity = (left: string, right: string) => {
  ...
  // Very short names are intentionally not fuzzy matched because a single
  // character can represent a materially different medicine.
  if (shortestLength < 5) return null;

  // A different first letter is treated as too uncertain for automatic
  // typo expansion. The pharmacist can still find the medicine manually.
...
const getMedicineReferenceNames = async (sourceName: string) => {
  ...
  const referenceMatches = references
    .map(reference => {
      ...
      return {
        reference,
        similarity:
          fuzzySimilarities.length > 0
            ? Math.max(...fuzzySimilarities)
            : 0,
        exact: false,
      };
    })
  ...
  if (!best.exact && !safelyUniqueFuzzyReference) {
...
const getNameMatch = (
  ...
  const possibleNames = Array.from(
    new Set([normalizedSource, ...acceptedNames]),
  );

  const fuzzyMatches = possibleNames
    .map(candidateName => ({
      candidateName,
      similarity: getSafeFuzzySimilarity(
        normalizedInventory,
        candidateName,
      ),
    }))
  ...
  const lengthRatio =
    longerLength > 0 ? shorterLength / longerLength : 0;

  if (
    shorterLength >= 5 &&
    lengthRatio >= 0.65 &&
    (normalizedInventory.includes(normalizedSource) ||
      normalizedSource.includes(normalizedInventory))
  ) {
    return {
...
const buildCandidate = (
  ...
  const strongMedicineIdentity =
    nameMatch.type === "EXACT" || nameMatch.type === "ALIAS";
  ...
  if (availableQuantity === 0) {
    reasons.push("No currently available stock.");
  }
...
const getOrderItemForPharmacy = async (
  pharmacyId: string,
  orderId: string,
  orderItemId: string,
) => {
...
export const ensureOrderInventoryReadyForFulfilment = async (
  ...
  // Keep legacy/manual orders without structured order items compatible.
  ...
  for (const reservation of reservations) {
    ...
    const released = await tx.pharmacyInventoryItem.updateMany({
      where: {
        ...
        pharmacyId,
        reservedQuantity: {
          gte: reservation.inventoryReservedQuantity,
        },
      },
      data: {
        reservedQuantity: {
          decrement: reservation.inventoryReservedQuantity,
        },
      },
    });
...
export const pharmacyInventoryMatchService = {
  async getCandidates(
    ...
    const { item } = await getOrderItemForPharmacy(
      pharmacyId,
      orderId,
      orderItemId,
    );
    ...
    const candidates = inventoryItems
      ...
      .filter(
        (
          candidate,
        ): candidate is PharmacyInventoryMatchCandidate & { score: number } =>
          candidate !== null,
      )
      .sort((a, b) => {
    ...
    const suggestedCandidateId =
      exactCandidates.length === 1 ? exactCandidates[0].id : null;
        ...
        if (oldReservationIsActive && item.inventoryItemId) {
          const released = await tx.pharmacyInventoryItem.updateMany({
            where: {
              ...
              pharmacyId,
              reservedQuantity: {
                gte: item.inventoryReservedQuantity,
              },
            },
            data: {
              reservedQuantity: {
                decrement: item.inventoryReservedQuantity,
              },
            },
          });
  ...
  async releaseMatch(
    ...
    const { order } = await getOrderItemForPharmacy(
      pharmacyId,
      orderId,
      orderItemId,
    );
