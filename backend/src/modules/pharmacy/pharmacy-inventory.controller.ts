...
import {
  ...
  pharmacyInventoryReferencePriceQuerySchema,
...
export const pharmacyInventoryController = {
  async listInventory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!parsed.success) throw new AppError(validationMessage(parsed.error), 400);

      const data = await pharmacyInventoryService.listInventory(getPharmacyId(req), parsed.data);
  ...
  async getReferencePrice(req: Request, res: Response, next: NextFunction) {
    try {
      const parsed = pharmacyInventoryReferencePriceQuerySchema.safeParse(req.query);
      if (!parsed.success) throw new AppError(validationMessage(parsed.error), 400);

      const data = await pharmacyInventoryService.getReferencePrice(
        getPharmacyId(req),
        parsed.data.medicineName,
        parsed.data.strength,
      );

      return res.status(200).json({
        success: true,
        message: data.found ? "Default medicine price found" : "No default medicine price found",
        data,
      });
    } catch (error) {
      next(error);
  ...
  async createInventoryItem(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await pharmacyInventoryService.createInventoryItem(getPharmacyId(req), parsed.data);
  ...
  async updateInventoryItem(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!params.success) throw new AppError(validationMessage(params.error), 400);
      if (!body.success) throw new AppError(validationMessage(body.error), 400);

      const data = await pharmacyInventoryService.updateInventoryItem(getPharmacyId(req), params.data.itemId, body.data);
  ...
  async archiveInventoryItem(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await pharmacyInventoryService.archiveInventoryItem(getPharmacyId(req), parsed.data.itemId);
  ...
  async restoreInventoryItem(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await pharmacyInventoryService.restoreInventoryItem(getPharmacyId(req), parsed.data.itemId);
