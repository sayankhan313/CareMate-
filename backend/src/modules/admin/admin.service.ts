...
const getEscalatedMedicineReviewOrThrow = async (requestId: string) => {
  ...
  if (!request) {
    throw new AppError("Admin medicine review escalation not found", 404);
  }
...
const getCompletedPoolReviewOrThrow = async (requestId: string) => {
  ...
  if (!request) {
    throw new AppError(
      "Completed Medicine Review Doctor Pool request not found",
      404
    );
  }
...
const getEligibleMedicineReviewDoctors = async (request: any) => {
  ...
  const poolWorkloadMap = new Map(
    poolWorkloads
      .filter(item => item.poolDoctorId)
      .map(item => [
        item.poolDoctorId as string,
        item._count._all,
      ])
  );
  ...
  return doctors
    .filter(
      doctor =>
        availabilityMap.get(doctor.id) !== "OUT_OF_OFFICE"
    )
    .map(doctor => {
      const normalReviews =
        normalWorkloadMap.get(doctor.id) || 0;

      const poolReviews =
        poolWorkloadMap.get(doctor.id) || 0;
      ...
      return {
        ...
        availabilityStatus:
          ...
          "NO_CALENDAR_ENTRY",
        isAlreadyAssigned:
          assignmentMap.has(doctor.id),
        assignmentType:
          assignmentMap.get(doctor.id) || null,
        pendingAssignedReviews:
          normalReviews,
        pendingPoolReviews:
          poolReviews,
        pendingMedicineReviews:
          normalReviews + poolReviews,
      };
    })
    .sort((first, second) => {
      if (
        first.pendingMedicineReviews !==
        second.pendingMedicineReviews
      ) {
        return (
          first.pendingMedicineReviews -
          second.pendingMedicineReviews
  ...
  try {
    ...
    await notificationService.createAndSend({
      ...
      title: approved
        ...
        : `${roleLabel} verification declined`,
      body: approved
    ...
    console.warn(
      `Unable to send account decision notification for user ${user.id}:`,
...
const notifyPoolDoctorAboutMedicineReview = async (
  requestId: string
) => {
  try {
    const request =
...
const notifyPatientAboutReleasedPoolReview = async (
  requestId: string
) => {
  try {
    const request =
    ...
    const doctorName =
      getDoctorDisplayName(
        request.reviewedByDoctor?.fullName
      );
    ...
    await notificationService.createAndSend({
      ...
      type: notificationType,
      title: approved
        ? "Medicine review approved"
        : "Medicine review declined",
      body,
      priority: "HIGH",
      entityType: "MEDICINE_REVIEW_REQUEST",
      entityId: request.id,
      ...
      data: {
        ...
        reviewedAt:
          ...
          null,
        source:
          "ADMIN_RELEASED_POOL_MEDICINE_REVIEW",
      },
    });
  } catch (error) {
    console.warn(
...
export const adminService = {
  async getDashboard() {
    ...
    return {
      ...
      recentDoctorVerifications:
        recentDoctorVerifications.map(
          formatDoctorVerification
        ),
      recentPharmacyVerifications:
        recentPharmacyVerifications.map(
  ...
  async listMedicineReviewEscalations() {
    ...
    return {
      ...
      requests:
        requests.map(formatMedicineReview),
    };
  },

  async getMedicineReviewEscalation(
    requestId: string
  ) {
    const request =
      await getEscalatedMedicineReviewOrThrow(
        requestId
      );

    const doctors =
      await getEligibleMedicineReviewDoctors(
        request
      );

    return {
      request:
        formatMedicineReview(request),
      eligibleDoctors: doctors,
    };
  },

  async assignMedicineReviewEscalation(
    requestId: string,
    doctorId: string
  ) {
    const request =
    ...
    if (
      request.attemptedDoctorIds.includes(
        doctorId
      )
    ) {
      throw new AppError(
    ...
    if (!doctor) {
      throw new AppError(
        "Selected doctor is not active and approved.",
        404
      );
    }
    ...
    const assigned =
      await prisma.$transaction(
        async tx => {
          const updated =
            await tx.medicineReviewRequest.updateMany({
              where: {
                id: request.id,
                status: "PENDING",
                routingStatus:
    ...
    if (!assigned) {
      throw new AppError(
        "Medicine review could not be assigned.",
        500
      );
    }

    await notifyPoolDoctorAboutMedicineReview(
      request.id
    );
  ...
  async releaseMedicineReviewPoolResult(
    requestId: string,
    adminId: string
  ) {
    const request =
      await getCompletedPoolReviewOrThrow(
        requestId
      );

    if (
      !request.poolDoctorId ||
      !request.poolDecision
    ) {
      throw new AppError(
        "Medicine Review Doctor Pool result is incomplete.",
        400
      );
    }

    const reviewedAt =
      request.poolReviewedAt ||
      new Date();
    ...
    await prisma.$transaction(async tx => {
      ...
      if (updated.count === 0) {
        throw new AppError(
          "Medicine review result has already been released.",
          409
        );
      }
    ...
    await notifyPatientAboutReleasedPoolReview(
      request.id
    );
  ...
  async approveDoctorVerification(
    ...
    if (
      ...
      throw new AppError(
        "Disabled doctor accounts cannot be approved",
        400
      );
    }
  ...
  async rejectDoctorVerification(
    ...
    if (
      ...
      throw new AppError(
        "Active doctor accounts cannot be rejected",
        400
      );
    }
    ...
    if (
      ...
      throw new AppError(
        "Disabled doctor accounts cannot be rejected",
        400
      );
    }
  ...
  async approvePharmacyVerification(
    ...
    if (
      ...
      throw new AppError(
        "Pharmacy account is already active",
        400
      );
    }
  ...
  async rejectPharmacyVerification(
    ...
    if (
      ...
      throw new AppError(
        "Active pharmacy accounts cannot be rejected",
        400
      );
    }
    ...
    if (
      ...
      throw new AppError(
        "Disabled pharmacy accounts cannot be rejected",
        400
      );
    }
