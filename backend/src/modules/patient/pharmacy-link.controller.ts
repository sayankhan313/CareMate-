...
const CHARGE_PREFERENCES = new Set<PrescriptionChargePreference>(["CHARGEABLE", "EXEMPT", "PPC"]);
...
const getPharmacyIdFromParams = (req: Request) => {
  ...
  if (typeof pharmacyId !== "string" || !pharmacyId.trim()) throw new AppError("Pharmacy ID is required", 400);
  return pharmacyId.trim();
};

const getChargePreferenceFromBody = (req: Request): PrescriptionChargePreference => {
  const value = req.body?.chargePreference;

  if (typeof value !== "string" || !CHARGE_PREFERENCES.has(value as PrescriptionChargePreference)) {
    throw new AppError("chargePreference must be CHARGEABLE, EXEMPT, or PPC", 400);
  }
...
const getLimitFromQuery = (req: Request) => {
  ...
  if (rawLimit === undefined) return 30;
  if (typeof rawLimit !== "string") throw new AppError("Limit must be a number", 400);
...
export const pharmacyLinkController = {
  async listApprovedPharmacies(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await pharmacyLinkService.listApprovedPharmacies(patientId, {
        search,
        city,
        postcode,
        limit,
  ...
  async listSavedPharmacies(req: Request, res: Response, next: NextFunction) {
    try {
      const patientId = getPatientId(req);
      const result = await pharmacyLinkService.listSavedPharmacies(patientId);
  ...
  async savePharmacy(req: Request, res: Response, next: NextFunction) {
    try {
      const patientId = getPatientId(req);
      const pharmacyId = getPharmacyIdFromParams(req);
      const chargePreference = getChargePreferenceFromBody(req);

      const result = await pharmacyLinkService.savePharmacy(patientId, pharmacyId, {
        chargePreference,
      });
  ...
  async setPrimaryPharmacy(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await pharmacyLinkService.setPrimaryPharmacy(patientId, pharmacyId);
  ...
  async updateChargePreference(req: Request, res: Response, next: NextFunction) {
    try {
      const patientId = getPatientId(req);
      const pharmacyId = getPharmacyIdFromParams(req);
      const chargePreference = getChargePreferenceFromBody(req);

      const result = await pharmacyLinkService.updateChargePreference(
        patientId,
        pharmacyId,
        chargePreference
      );
      ...
      return res.status(200).json({
        success: true,
        message: "Prescription charge preference updated successfully",
        data: result,
      });
    } catch (error) {
      next(error);
  ...
  async removePharmacy(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await pharmacyLinkService.removePharmacy(patientId, pharmacyId);
