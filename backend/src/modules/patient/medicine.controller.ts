...
export const medicineController = {
  async createMedicine(
    ...
    return res.status(201).json({
      success: true,
      message:
        validatedData.sendToDoctorForReview
          ? "Medicine sent to your primary doctor for review"
  ...
  async listMedicines(
    ...
    const result =
      await medicineService.listMedicines(
        patientId
      );
  ...
  async getTodayMedicines(
    ...
    const result =
      await medicineService.getTodayMedicines(
        patientId
      );
    ...
    return res.status(200).json({
      success: true,
      message:
        "Today's medicines fetched successfully",
      data: result,
    });
  },
  ...
  async requestMedicineDeletion(
    ...
    const validatedData =
      requestMedicineDeletionSchema.parse(
        req.body
      );

    const result =
      await medicineService.requestMedicineDeletion(
        patientId,
        params.medicineId,
        validatedData
      );
  ...
  async listMedicineReviews(
    ...
    const result =
      await medicineService.listMedicineReviewRequests(
        patientId
      );
    ...
    return res.status(200).json({
      success: true,
      message:
        "Medicine review updates fetched successfully",
      data: result,
    });
  },
  ...
  async markMedicineReviewSeen(
    ...
    const params =
      medicineReviewRequestIdParamsSchema.parse(
        req.params
      );

    const result =
      await medicineService.markMedicineReviewSeen(
        patientId,
        params.requestId
      );
    ...
    return res.status(200).json({
      success: true,
      message:
        "Medicine review marked as seen",
      data: result,
    });
  },
  ...
  async applyApprovedMedicineReview(
    ...
    const result =
      await medicineService.applyApprovedMedicineReview(
        patientId,
        params.requestId
      );
  ...
  async resubmitMedicineReview(
    ...
    const validatedData =
      resubmitMedicineReviewSchema.parse(
        req.body
      );

    const result =
      await medicineService.resubmitMedicineReview(
        patientId,
        params.requestId,
        validatedData
      );
  ...
  async snoozeReminder(
    ...
    const validatedData =
      snoozeMedicineSchema.parse(
        req.body
      );

    const result =
      await medicineService.snoozeReminder(
        patientId,
        params.reminderId,
        validatedData
      );
